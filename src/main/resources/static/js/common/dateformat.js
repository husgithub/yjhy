...
(function () {
	...
	var _date_format_api = {
         ...
         * @param date
         */
        getDateOnlyStr: function (date, fmt) {
            if (typeof fmt == 'undefined') {
                fmt = 'yyyy-MM-dd';
            }
            var dateStr = this.format(date, fmt);
        ...
        /**
		 * 将struts2默认时间字符串转成相应格式时间字符串
         * @param dateStr
         * @param fmt
         * @returns {*}
         */
		formatStrutsDate : function(dateStr,fmt){
			if(!this.isNotEmpty(dateStr)){
				return '';
			}
			...
			return this.format(this.formatToDate(dateStr),fmt);
		},
        /**
		 * 两时间字符串相减
         * @param startTimeStr
         ...
         * @returns {number}
         */
        subtract : function(startTimeStr,endTimeStr){
            if(!this.isNotEmpty(startTimeStr) || !this.isNotEmpty(endTimeStr)){
                return this.formatTimeToHumanStr(0);
            }
            startTimeStr = startTimeStr.indexOf("T")!=0?startTimeStr.replace(/T/g,   "  "):startTimeStr;
            endTimeStr = endTimeStr.indexOf("T")!=0?endTimeStr.replace(/T/g,   "  "):endTimeStr;
            var startDate = this.formatToDate(startTimeStr);
            var endDate = this.formatToDate(endTimeStr);
            return this.formatTimeToHumanStr(Math.abs(endDate.getTime()-startDate.getTime()));
        },
        /**
		 * 格式化时间毫秒返回字符串
         * @param time
         * @returns {string}
         */
		formatTimeToHumanStr : function (time) {
            time = time/1000;
			var result = "";
			if(time >0 && time <60){
                result = time+"秒";
			}else if(time>=60 && time < 60*60){    //小于一小时
				var minute = Math.floor(time/60);
                time = time - minute*60;
				result = minute+"分"+time+"秒";
			}else if(time>=3600 && time <24*60*60){  //24小时内
				var hour = Math.floor(time/3600);
				time = time-hour*3600;
                var minute = Math.floor(time/60);
                time = time - minute*60;
                result = hour+"小时"+minute+"分"+time+"秒";
